...
echo "≡ƒöä Updating Klipper Adaptive Flow..."
echo ""

cd "$REPO_DIR"

# Pull latest changes
echo "≡ƒôÑ Fetching updates from GitHub..."
git pull

# Copy Python modules (always safe to update)
echo "≡ƒôü Updating Python modules..."
...
echo "≡ƒôï Updating system defaults..."
...
if [ ! -f "$USER_CFG" ]; then
    echo ""
    echo "ΓÜá∩╕Å  First-time setup detected!"
    echo "≡ƒô¥ Creating user configuration template..."
    cp auto_flow_user.cfg.example "$USER_CFG"
    echo "Γ£à Created: $USER_CFG"
    echo ""
    echo "≡ƒô¥ Edit $USER_CFG to customize your settings"
fi

if [ ! -f "$USER_MAT" ]; then
    echo "≡ƒô¥ Creating user material profiles template..."
    cp material_profiles_user.cfg.example "$USER_MAT"
    echo "Γ£à Created: $USER_MAT"
fi
...
if [ -f "$OLD_AUTO" ] && [ ! -L "$OLD_AUTO" ]; then
    ...
    echo "Γ£à Backup saved to: $BACKUP_FILE"
    echo ""
...
echo ""
echo "≡ƒôï Make sure your printer.cfg includes:"
...
echo ""
echo "≡ƒöä Restarting Klipper..."
sudo systemctl restart klipper

echo ""
echo "Γ£à All done! Your custom settings are preserved."
